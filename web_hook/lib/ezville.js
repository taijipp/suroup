...
const Events = (key, value, component='main') => {
	let event;
	switch(key) {
		case 'switch':
			event = {
			...
			if( packet.devId == TYPE.LIGHT ) {
				for(let i=1, max=packet.len; i<max; i++){
					...
					const device = {
						...
						//uri: '/homenet/'+id,
						states:[
							Events('switch', parseInt(state, 16)?'on':'off')
						],
						property: {
			...
			if( packet.devId == TYPE.THERMOSTAT ) {
				...
				for(let i=0, max=thermostats.length; i<max; i++){
					...
					const device = {
						...
						//uri: '/homenet/'+id,
						states:[
							Events('switch', mode=='heat'?'on':'off'),
							Events('setTemp', setTemp),
							Events('curTemp', curTemp),
						],
						property: {
...
const thermostat = (id, socket) => {
	...
	return {
		//set mode(value) {
		//	send(socket)(src + (value=='heat'?'430101':'450101'));
		set switch(value) {
