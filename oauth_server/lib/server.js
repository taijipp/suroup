...
class OauthServer {
    ...
    constructor(config = {}) {
		...
		const { code={}, refreshData={}, authHeaderData={}, tokenIdData={}, redirectUri='' } = db.load();
        this.code = code;
        this.refreshData = refreshData;
        this.authHeaderData = authHeaderData;
        this.tokenIdData = tokenIdData;
    ...
    setRoutes() {
        this.expressApp
            .get('/oauth/login', this.authenticationCallback.bind(this))
            .get('/oauth/auth', this.loginAsCallback.bind(this))
            .post('/oauth/token', this.tokenCallback.bind(this));
	...
	saveSession() {
		db.save({
			...
			authHeaderData: this.authHeaderData,
			tokenIdData: this.tokenIdData,
    ...
    loginAsCallback(request, response) {
		...
		const { account_email, account_pass } = process.env;
		if( account_pass == "" || name != account_pass || 
