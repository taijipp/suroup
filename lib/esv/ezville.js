...
const isSocketWritable = socket => socket && !socket.destroyed && socket.writable;
...
const esv = (id, socket) => {
	...
	return {
		...
		set fanSpeed(value) {
			const speed = value & 0xff;
			const fanSpeed = _.padStart(speed.toString(16), 2, '0');
			send(socket)(src + fanSpeed + '00' + (speed === 0 ? '00' : '03'));
		},
		set switch(value) {
...
const send = (socket) => (hexString, checksum=true) => {
	if( !isSocketWritable(socket) ) {
		console.log('ESV - command skipped because socket is not connected.');
		return;
	}
	...
	function intervalFunc() {
		...
		if( !isSocketWritable(socket) || !_.isEmpty(response) || count >= 7) {
			_.unset(ack, addr);
			clearInterval(this);
		} else {
			count++;
