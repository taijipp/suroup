			...
			if( packet.devId == TYPE.POWEROUTLET ) {
				...
				for(let i=0, max=outlets.length; i<max; i++){
					...
					if( _.get(ratelimit, id, 0) < Number(Date.now()) ) {
						ratelimit[id] = Number(Date.now()) + 5000;
						_.set(devices, device.id, device);
					}
				}
...
const light = (id, socket) => {
	...
	return {
		set switch(value) {
			send(socket)(src + (value=='on'?'0100':'0000'), true);
		}
	}
}
...
const thermostat = (id, socket) => {
	...
	return {
		set mode(value) {
			send(socket)(src + (value=='heat'?'430101':'450101'), true);
		},
		set setTemp(temp) {
			send(socket)(src + '4401' + (temp & 0xff).toString(16), true);
		}
	}
}
...
const outlet = (id, socket) => {
	...
	return {
		set switch(value) {
			send(socket)(src + (value=='on'?'11':'10'), true);
		}
	}
}
...
const send = (socket) => (hexString, checksum) => {
	if( checksum ) {
...
module.exports = {chop, parsing, save, setup, light, thermostat, outlet, gas, send };
